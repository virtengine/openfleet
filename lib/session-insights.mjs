...
export function buildSessionInsights(fullSession = null) {
  ...
  const shortSummarySteps = [];
  let currentStep = null;

  const flushCurrentStep = () => {
    if (!currentStep) return;
    const label = String(currentStep.label || "").trim();
    if (!label) {
      currentStep = null;
      return;
    }
    shortSummarySteps.push({
      index: shortSummarySteps.length + 1,
      label,
      kind: currentStep.kind || "activity",
      timestamp: currentStep.timestamp || null,
      toolName: currentStep.toolName || null,
      file: currentStep.file || null,
      count: Number(currentStep.count || 1),
    });
    currentStep = null;
  };

  const pushStep = (kind, label, extra = {}) => {
    const normalizedLabel = String(label || "").replace(/\s+/g, " ").trim();
    if (!normalizedLabel) return;
    const timestamp = String(extra.timestamp || "").trim() || null;
    const toolName = String(extra.toolName || "").trim() || null;
    const file = String(extra.file || "").trim() || null;
    if (
      currentStep &&
      currentStep.kind === kind &&
      currentStep.label === normalizedLabel &&
      currentStep.toolName === toolName &&
      currentStep.file === file
    ) {
      currentStep.count += 1;
      currentStep.timestamp = timestamp || currentStep.timestamp;
      return;
    }
    flushCurrentStep();
    currentStep = {
  ...
  for (const msg of messages) {
    ...
    if (type === "error" || type === "stream_error") {
      errors += 1;
      pushStep("error", content.slice(0, 120) || "Error reported", { timestamp: ts });
      recentActions.push({
        type: "error",
    ...
    if (type === "tool_result" || type === "tool_output") {
      toolResults += 1;
      pushStep("tool_result", "Observed tool result", { timestamp: ts });
      continue;
    }
    ...
    if (classification.kind === "open") {
      ...
      pushStep(
        "open",
        combinedFiles[0] ? `Inspected ${combinedFiles[0]}` : `Inspected with ${toolName}`,
        { timestamp: ts, toolName, file: combinedFiles[0] || null },
      );
      ...
      pushStep(
        "edit",
        combinedFiles[0] ? `Edited ${combinedFiles[0]}` : `Changed files with ${toolName}`,
        { timestamp: ts, toolName, file: combinedFiles[0] || null },
      );
    } else {
      pushStep("tool_call", `Used ${toolName}`, { timestamp: ts, toolName });
    }
  ...
  const trajectory = messages.map((msg, index) => ({
    index: index + 1,
    id: msg?.id || null,
    type: String(msg?.type || "message"),
    role: String(msg?.role || ""),
    timestamp: String(msg?.timestamp || "") || null,
    content: toText(msg?.content).slice(0, 4000),
    meta: msg?.meta && typeof msg.meta === "object" ? { ...msg.meta } : null,
  }));

  const overview = [];
  if (editedFiles.size > 0) overview.push(`Edited ${editedFiles.size} file${editedFiles.size === 1 ? "" : "s"}`);
  if (openedFiles.size > 0) overview.push(`inspected ${openedFiles.size} file${openedFiles.size === 1 ? "" : "s"}`);
  if (toolCalls > 0) overview.push(`used ${toolCalls} tool${toolCalls === 1 ? "" : "s"}`);
  if (errors > 0) overview.push(`${errors} error${errors === 1 ? "" : "s"}`);
  ...
  const derived = {
    ...
    replay: {
      trajectory,
      steps: shortSummarySteps.slice(0, 50),
      resumeHint: shortSummarySteps.at(-1)?.label || null,
      overview: overview.length ? overview.join(", ") : null,
    },
  ...
  return {
    ...
    replay: persisted.replay || derived.replay,
