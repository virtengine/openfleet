...
function buildTurnTimeline(messages = []) {
  ...
  for (const msg of Array.isArray(messages) ? messages : []) {
    ...
    const content = toText(msg.content).replace(/\s+/g, " ").trim();
    if (role === "assistant" || type === "agent_message" || type === "assistant_message") {
      entry.assistantMessages += 1;
      if (!entry.assistantPreview) {
        entry.assistantPreview = content.slice(0, 180);
      }
      if (!entry.preview) {
        entry.preview = content.slice(0, 140);
      }
    } else if ((type === "error" || type === "stream_error") && !entry.preview) {
      entry.preview = content.slice(0, 140);
    }
  ...
  return Array.from(turns.values())
    ...
    .map((entry) => {
      ...
      return {
        ...entry,
        durationMs: startedMs !== null && endedMs !== null ? Math.max(0, endedMs - startedMs) : 0,
        ...
        preview: entry.preview || entry.assistantPreview || `Turn ${entry.turn}`,
      };
    });
}
...
export function buildSessionInsights(fullSession = null) {
  ...
  const turnTimeline = buildTurnTimeline(messages);
  const derived = {
    totals: {
  ...
  return {
    ...
    turnTimeline: Array.isArray(persisted.turnTimeline)
      ? persisted.turnTimeline
      : (Array.isArray(persisted.turns) ? persisted.turns : derived.turnTimeline),
    turns: Array.isArray(persisted.turns)
      ? persisted.turns
      : (Array.isArray(persisted.turnTimeline) ? persisted.turnTimeline : derived.turns),
