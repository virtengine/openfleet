...
function trimWrappingQuotes(text) {
  ...
  let start = 0;
  let end = text.length;
  while (start < end && markers.includes(text[start])) start++;
  while (end > start && markers.includes(text[end - 1])) end--;
  return text.slice(start, end);
}

function normalizeFilePath(raw) {
  const text = trimWrappingQuotes(String(raw || "").trim());
  if (!text) return "";
...
function extractFilesFromPatchText(text) {
  ...
  for (const line of source.split(/\r?\n/)) {
    if (line.startsWith("*** Update File:")) {
      const file = normalizeFilePath(line.slice("*** Update File:".length).trim());
      if (file) files.push(file);
      continue;
    }
    if (line.startsWith("*** Add File:")) {
      const file = normalizeFilePath(line.slice("*** Add File:".length).trim());
      if (file) files.push(file);
      continue;
    }
    if (line.startsWith("*** Delete File:")) {
      const file = normalizeFilePath(line.slice("*** Delete File:".length).trim());
      if (file) files.push(file);
      continue;
    }
    if (line.startsWith("*** Move to:")) {
      const file = normalizeFilePath(line.slice("*** Move to:".length).trim());
      if (file) files.push(file);
    }
  }
  return files;
...
function trimEdgePunctuation(text) {
  ...
  let start = 0;
  let end = text.length;
  while (start < end && punct.includes(text[start])) start++;
  while (end > start && punct.includes(text[end - 1])) end--;
  return text.slice(start, end);
}
...
function extractFilesFromText(text) {
  ...
  for (const token of source.split(/\s+/)) {
    const cleaned = trimEdgePunctuation(token);
...
function parseContextWindowSnapshot(text) {
  ...
  const tokensIdx = lower.indexOf("tokens");
  if (tokensIdx >= 0) {
    const prefix = source.slice(Math.max(0, tokensIdx - 80), tokensIdx);
    const slashIdx = prefix.lastIndexOf("/");
    if (slashIdx >= 0) {
      const usedRaw = prefix.slice(0, slashIdx).trim().split(/\s+/).pop() || "";
      const totalRaw = prefix.slice(slashIdx + 1).trim().split(/\s+/)[0] || "";
      const usedTokens = parseCompactNumber(usedRaw);
      const totalTokens = parseCompactNumber(totalRaw);
      let percent = null;
      const pct = source.slice(tokensIdx).match(/(\d+(?:\.\d+)?)\s*%/);
      if (pct) percent = Number(pct[1]);
      if (!Number.isFinite(percent) && usedTokens != null && totalTokens) {
        percent = Math.round((usedTokens / totalTokens) * 1000) / 10;
      }
      if (usedTokens != null || totalTokens != null || Number.isFinite(percent)) {
        return { usedTokens, totalTokens, percent: Number.isFinite(percent) ? percent : null };
      }
    }
  }
  ...
  const after = source.slice(markerIdx + marker.length);
  const pct = after.match(/(\d+(?:\.\d+)?)\s*%/);
  if (!pct) return null;
