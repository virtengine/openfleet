/**
 * logger.mjs — Centralized logging for bosun.
 *
...
const STDERR_NOISE_PATTERNS = [
  ...
  /^(?:Error:\s+)?write EOF\s*$/i,
];

function isBenignBrokenPipeError(err) {
...
export function installConsoleInterceptor(opts = {}) {
  ...
  process.stderr.on("error", (err) => {
    if (markStderrBroken(err)) {
      // Broken pipes happen during intentional restarts/daemon detach; avoid
      // surfacing benign stream closure as an error-log anomaly.
      if (logFilePath && LogLevel.WARN >= fileLevel) {
        writeToFile("WARN", "process", `stderr stream closed: ${err?.message || err}`);
      }
      return;
    }
  ...
  process.stdout.on("error", (err) => {
    if (markStdoutBroken(err)) {
      ...
      // surfacing benign stream closure as an error-log anomaly.
      if (logFilePath && LogLevel.WARN >= fileLevel) {
        writeToFile("WARN", "process", `stdout stream closed: ${err?.message || err}`);
      }
      return;
    }
