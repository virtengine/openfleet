...
function extractAnnotationLines(content) {
  ...
  const summaryLine = lines.find((line) => SUMMARY_MARKERS.some((marker) => line.includes(marker))) || "";
  const warnLines = lines.filter((line) => WARN_MARKERS.some((marker) => line.includes(marker)));
...
export function scanRepository(rootDir, options = {}) {
  ...
  function visit(currentDir) {
    ...
    for (const entry of entries) {
      ...
      const sourceType = getSourceType(entry.name);
      if (!sourceType) continue;
      if (allowedExtensions.length > 0 && !allowedExtensions.includes(extname(entry.name).toLowerCase())) continue;
  ...
  if (targetStat.isFile()) {
    ...
    if (getSourceType(targetDir) && !isGeneratedPath(relPath)) visit(dirname(targetDir));
  }
...
function hasNearbyWarn(lines, lineIndex) {
  ...
  for (let index = start; index <= end; index += 1) {
    if (WARN_MARKERS.some((marker) => lines[index].includes(marker))) return true;
  }
  return false;
}
...
function findStaleWarnings(content, language) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    if (!WARN_MARKERS.some((marker) => lines[index].includes(marker))) continue;
