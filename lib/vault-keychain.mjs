...
function escapePowerShellCommand(script) {
...
`.trim();
    ...
    const result = execSync(
      `powershell -NoProfile -NonInteractive -Command "${escapePowerShellCommand(ps)}"`,
...
function winWrite(hexKey) {
  try {
    const ps = `
$target = '${SERVICE_NAME}/${ACCOUNT_NAME}'
$pass = ConvertTo-SecureString '${hexKey}' -AsPlainText -Force
...
$blob = [System.Text.Encoding]::Unicode.GetBytes('${hexKey}')
...
function macWrite(hexKey) {
  try {
    ...
    execSync(
      `security add-generic-password -s "${SERVICE_NAME}" -a "${ACCOUNT_NAME}" -w "${hexKey}"`,
...
function linuxWrite(hexKey) {
  try {
    execSync(
      `echo -n "${hexKey}" | secret-tool store --label="Bosun Vault Master Key" service "${SERVICE_NAME}" account "${ACCOUNT_NAME}"`,
      { encoding: "utf8", timeout: 10_000, stdio: "ignore" }
    );
    return true;
  } catch {
