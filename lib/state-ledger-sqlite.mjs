...
export function resolveStateLedgerPath(options = {}) {
  const explicitPath = asText(options.ledgerPath || process.env.BOSUN_STATE_LEDGER_PATH || "");
  if (explicitPath) {
...
export function getTaskSnapshotFromStateLedger(taskId, options = {}) {
  return withLedger(options, (entry) => {
    ...
    return parseJsonText(row.document_json);
  });
}
...
export function listTaskSnapshotsFromStateLedger(options = {}) {
  return withLedger(options, (entry) => {
    ...
    return rows.map((row) => parseJsonText(row?.document_json)).filter(Boolean);
  });
}
