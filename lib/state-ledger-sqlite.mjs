...
/**
 * Lazy-load node:sqlite to avoid crashing on Node < 22.5 where the built-in
 * module does not exist.  Callers that never open an actual ledger (e.g. test
 * imports that only pull type-level helpers) will never trigger the import.
 */
let _DatabaseSync;
  ...
  if (!_DatabaseSync) {
    try {
      ...
      throw new Error(
        ...
        "SQLite state-ledger requires Node >= 22.5.0.",
      );
    }
  }
...
function normalizeWorkflowRunDocument(runDocument = {}) {
  ...
  return {
    ...
    taskTitle: taskIdentity.taskTitle,
    ...
    runTransaction(entry, () => {
      prepare(
        ...
        `INSERT INTO workflow_runs (
           ...
           task_id, task_title, session_id, session_type, event_count, document_json
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
