...
function normalizeTextTree(value) {
  if (Array.isArray(value)) {
    return value.map((entry) => normalizeTextTree(entry));
  }
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, normalizeTextTree(entry)]),
    );
  }
...
function buildPromptEntries() {
  ...
  return AGENT_PROMPT_DEFINITIONS.map((def, index) => {
    ...
    return {
      ...
      content: normalizeLineEndings(getDefaultPromptTemplate(def.key)),
    };
  });
}
...
function buildSkillEntries() {
  ...
  return BUILTIN_SKILLS.map((skill, index) => ({
    ...
    content: normalizeLineEndings(skill.content).trim() + "\n",
  }));
}
...
export function renderDefaultsScript(data) {
  const payload = JSON.stringify(normalizeTextTree(data), null, 2);
  return [
