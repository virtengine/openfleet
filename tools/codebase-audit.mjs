  ...
  switch (parsed.command) {
    ...
    case "conformity": {
      ...
      if (parsed.json) {
        ...
        for (const path of result.report.missingSummaries) {
          console.log(`WARN missing-summary ${path}`);
        }
        for (const path of result.report.missingWarnings) {
          console.log(`WARN missing-warning ${path}`);
        }
        for (const entry of result.report.staleAnnotations) {
          console.log(`WARN stale-annotation ${entry.path} (${entry.symbol})`);
        }
        for (const leak of result.report.credentialLeaks) {
          console.log(`WARN credential-leak ${leak.path} (${leak.kind}@L${leak.line})`);
        }
