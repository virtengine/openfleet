...
export const GITHUB_KANBAN_SYNC_TEMPLATE = {
  ...
  nodes: [
    ...
    node("fetch-pr-state", "action.run_command", "Fetch Bosun PR State", {
      command: "node tools/github-kanban-sync-fetch-pr-state.mjs",
      continueOnError: true,
      env: {
        BOSUN_GITHUB_SYNC_LOOKBACK_HOURS: "{{lookbackHours}}",
        BOSUN_GITHUB_SYNC_REPO_SCOPE: "{{repoScope}}",
      },
    }, { x: 400, y: 200 }),
    ...
    node("sync-programmatic", "action.run_command", "Sync PR State → Kanban (Programmatic)", {
      command: "node tools/github-kanban-sync-programmatic.mjs",
      continueOnError: true,
      failOnError: false,
      env: {
