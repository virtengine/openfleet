  ...
  nodes: [
    ...
    node("programmatic-review", "action.run_command", "Review Gate: Merge Single PR", {
      command: [
        ...
        "  try{",
        "    const stateRaw=gh(['pr','view',n,'--repo',repo,'--json','mergeStateStatus']);",
        "    const stateObj=(()=>{try{return JSON.parse(stateRaw||'{}')}catch{return {}}})();",
        "    if(stateObj.mergeStateStatus==='BEHIND'){",
        "      gh(['api','repos/'+repo+'/pulls/'+n+'/update-branch','--method','PUT','-f','update_method=merge']);",
        "      console.error('[progressor] updated behind branch for PR #'+n);",
        "      console.log(JSON.stringify({mergedCount:0,heldCount:0,skippedCount:1,skipped:[{repo,number:n,reason:'branch_updated_awaiting_ci'}]}));process.exit(0);",
        "    }",
        "  }catch(updateErr){console.error('[progressor] branch-update check for PR #'+n+': '+String(updateErr?.message||updateErr).slice(0,120));}",
  ...
  nodes: [
    ...
    node("programmatic-review", "action.run_command", "Review Gate: Programmatic Merge", {
      command: [
        ...
        "      if(stateObj.mergeStateStatus==='BEHIND'){",
        "        gh(['api','repos/'+repo+'/pulls/'+n+'/update-branch','--method','PUT','-f','update_method=merge']);",
        "        console.error('[watchdog] updated behind branch for PR #'+n);",
        "        skipped.push({repo,number:n,reason:'branch_updated_awaiting_ci'});continue;",
        "      }",
        "    }catch(updateErr){console.error('[watchdog] branch-update check for PR #'+n+': '+String(updateErr?.message||updateErr).slice(0,120));}",
        "    const mergeArgs=['pr','merge',n,'--repo',repo,'--delete-branch'];",
        "    if(method==='rebase') mergeArgs.push('--rebase');",
        "    else if(method==='merge') mergeArgs.push('--merge');",
        "    else mergeArgs.push('--squash');",
