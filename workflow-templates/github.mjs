  ...
  nodes: [
    ...
    node("programmatic-review", "action.run_command", "Review Gate: Merge Single PR", {
      command: [
        ...
        "      console.log(JSON.stringify({mergedCount:0,heldCount:0,skippedCount:1,skipped:[{repo,number:n,reason:'branch_behind_updating'}]}));process.exit(0);",
        "    }",
        "  }catch(ubErr){console.error('[progressor] update-branch check for PR #'+n+': '+String(ubErr?.message||ubErr).slice(0,120));}",
  ...
  nodes: [
    ...
    node("programmatic-review", "action.run_command", "Review Gate: Programmatic Merge", {
      command: [
        ...
        "        skipped.push({repo,number:n,reason:'branch_behind_updating'});continue;",
        "      }",
        "    }catch(ubErr){console.error('[watchdog] update-branch check for PR #'+n+': '+String(ubErr?.message||ubErr).slice(0,120));}",
        "    const mergeArgs=['pr','merge',n,'--repo',repo,'--delete-branch'];",
        "    if(method==='rebase') mergeArgs.push('--rebase');",
        "    else if(method==='merge') mergeArgs.push('--merge');",
        "    else mergeArgs.push('--squash');",
