  ...
  nodes: [
    ...
    node("fetch-and-classify", "action.run_command", "Fetch, Classify & Label PRs", {
      ...
      command: [
        ...
        "    }",
        "  } else {",
        "    // No failures, no conflicts — PR is clean or has only pending checks.",
        "    // Self-heal: if bosun-needs-fix is present but no active failures/conflicts,",
        "    // the label is stale (e.g. repair agent fixed the branch but forgot to remove",
        "    // it, or CI was suppressed so the label was never cleared). Remove it so the",
        "    // PR can flow to the merge path on the next watchdog cycle.",
        ...
        "    }",
        "    // CI kick: if no check runs exist, CI was never triggered (e.g. suppressed by",
        "    // GITHUB_TOKEN). Kick it via workflow_dispatch so checks appear next cycle.",
