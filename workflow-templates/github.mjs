  ...
  nodes: [
    ...
    node("programmatic-review", "action.run_command", "Review Gate: Merge Single PR", {
      command: [
        ...
        "  try{",
        "    const mrgState=gh(['pr','view',n,'--repo',repo,'--json','mergeStateStatus','--jq','.mergeStateStatus']);",
        "    if(mrgState==='BEHIND'){",
        "      try{",
        "        gh(['api','repos/'+repo+'/pulls/'+Number(n)+'/update-branch','-X','PUT','-f','update_method=merge']);",
        "        console.error('[progressor] PR #'+n+' is behind base branch — triggered update-branch merge');",
        "        console.log(JSON.stringify({mergedCount:0,heldCount:0,skippedCount:1,skipped:[{repo,number:n,reason:'branch_behind_updating'}]}));",
        "      }catch(ubErr){",
        "        console.error('[progressor] update-branch failed for PR #'+n+': '+String(ubErr?.message||ubErr).slice(0,120));",
        "        console.log(JSON.stringify({mergedCount:0,heldCount:1,skippedCount:0,held:[{repo,number:n,reason:'branch_behind_update_failed',error:String(ubErr?.message||ubErr).slice(0,200)}]}));",
        "      }",
        "      process.exit(0);",
        "    }",
        "  }catch(ubErr){console.error('[progressor] mergeStateStatus check for PR #'+n+': '+String(ubErr?.message||ubErr).slice(0,120));}",
  ...
  nodes: [
    ...
    node("programmatic-review", "action.run_command", "Review Gate: Programmatic Merge", {
      command: [
        ...
        "      const mrgState=gh(['pr','view',n,'--repo',repo,'--json','mergeStateStatus','--jq','.mergeStateStatus']);",
        "      if(mrgState==='BEHIND'){",
        "        const prNum=Number(n);",
        "        try{",
        "          gh(['api','repos/'+repo+'/pulls/'+prNum+'/update-branch','-X','PUT','-f','update_method=merge']);",
        "          console.error('[watchdog] PR #'+n+' is behind base branch — triggered update-branch merge');",
        "          skipped.push({repo,number:n,reason:'branch_behind_updating'});",
        "        }catch(ubErr){",
        "          console.error('[watchdog] update-branch failed for PR #'+n+': '+String(ubErr?.message||ubErr).slice(0,120));",
        "          skipped.push({repo,number:n,reason:'branch_behind_update_failed'});",
        "        }",
        "        continue;",
        "      }",
        "    }catch(stateErr){console.error('[watchdog] mergeStateStatus check for PR #'+n+': '+String(stateErr?.message||stateErr).slice(0,120));}",
        "    const mergeArgs=['pr','merge',n,'--repo',repo,'--delete-branch'];",
        "    if(method==='rebase') mergeArgs.push('--rebase');",
        "    else if(method==='merge') mergeArgs.push('--merge');",
        "    else mergeArgs.push('--squash');",
