...
export const PR_QUALITY_SIGNAL_SNIPPET = [
  ...
  "const QUALITY_PENDING_STATES=new Set(['PENDING','IN_PROGRESS','QUEUED','WAITING','REQUESTED']);",
  "const QUALITY_CHECK_RE=/(^|[^a-z])(sonar|sonarqube|sonarcloud|eslint|typescript|tsc|lint|prettier|codacy|quality gate)([^a-z]|$)/i;",
  "const SONAR_CHECK_RE=/(^|[^a-z])(sonar|sonarqube|sonarcloud)([^a-z]|$)/i;",
  "const REVIEW_ACTION_RE=/\\b(fix|should|must|please|error|warning|issue|bug|failing|lint|typescript|ts\\d+|sonar|quality gate|address|resolve|cleanup)\\b/i;",
  "function qualitySafeGhJsonRunner(runner,args,fallback){try{const out=runner(args);return out?JSON.parse(out):fallback;}catch{return fallback;}}",
