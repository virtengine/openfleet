...
/* ═══════════════════════════════════════════════════════════════
 *  RepoEnvironmentPanel — per-repo environment configuration
 * ═══════════════════════════════════════════════════════════════ */
function RepoEnvironmentPanel() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [reposRes, tplRes] = await Promise.all([
        apiFetch("/api/repos/environment"),
        apiFetch("/api/env/templates"),
      ]);
      if (reposRes?.ok && Array.isArray(reposRes.repos)) {
        setRepos(reposRes.repos);
        if (!selectedRepo && reposRes.repos.length > 0) {
          const first = reposRes.repos[0];
          setSelectedRepo(first.name || first.slug || "");
          setEnv(first.environment ? { ...first.environment } : buildEmptyEnv());
        }
      }
      if (tplRes?.ok && Array.isArray(tplRes.templates)) {
        setTemplates(tplRes.templates);
        setTemplateGroups(tplRes.groups || []);
      }
    } catch { /* ignore */ }
    setLoading(false);
  }, [selectedRepo]);
  ...
  function buildEmptyEnv() {
    return { template: "", installCommands: [], startCommand: "", buildCommand: "", testCommand: "", lintCommand: "", debugCommand: "", worktreeSetupScript: "", sharedPaths: [] };
  }
  ...
  const handleRepoChange = useCallback((e) => {
    ...
    const repo = repos.find((r) => (r.name || r.slug) === name);
    setEnv(repo?.environment ? { ...repo.environment } : buildEmptyEnv());
  }, [repos]);

  const handleTemplateChange = useCallback((e) => {
    const id = e.target.value;
    const tpl = templates.find((t) => t.id === id);
    if (!tpl) { setEnv((f) => ({ ...f, template: id })); return; }
    setEnv({
      template: id,
  ...
  const handleAutoDetect = useCallback(async () => {
    ...
    setDetectedInfo(null);
    try {
      const repo = repos.find((r) => (r.name || r.slug) === selectedRepo);
      const res = await apiFetch("/api/env/detect", { method: "POST", body: JSON.stringify({ repoPath: repo?.path || null }) });
      if (res?.ok) {
        setDetectedInfo(res);
        if (res.environment) {
          setEnv({ ...buildEmptyEnv(), ...res.environment });
        } else if (res.stack) {
          setEnv((f) => ({
            ...f,
            installCommands: res.stack.commands?.install || [],
            startCommand: res.stack.commands?.start || f.startCommand,
            buildCommand: res.stack.commands?.build || f.buildCommand,
            testCommand: res.stack.commands?.test || f.testCommand,
            lintCommand: res.stack.commands?.lint || f.lintCommand,
            debugCommand: res.stack.commands?.debug || f.debugCommand,
          }));
        }
      }
    } catch { /* ignore */ }
  ...
  const handleSave = useCallback(async () => {
    if (!selectedRepo || !env) return;
    setSaving(true);
    try {
      const res = await apiFetch("/api/repos/environment", {
        method: "POST",
        body: JSON.stringify({ name: selectedRepo, environment: env }),
      });
      if (res?.ok) {
        showToast ? showToast("Environment saved", "success") : null;
        await loadData();
      } else {
        showToast ? showToast(res?.error || "Save failed", "error") : null;
      }
    } catch { /* ignore */ }
    setSaving(false);
  }, [selectedRepo, env, loadData]);

  const updateEnvField = (key) => (e) => setEnv((f) => ({ ...f, [key]: e.target.value }));

  const updateInstallCommand = (idx) => (e) => setEnv((f) => {
    const cmds = [...(f.installCommands || [])];
    cmds[idx] = e.target.value;
    return { ...f, installCommands: cmds };
  });
  const addInstallCommand = () => setEnv((f) => ({ ...f, installCommands: [...(f.installCommands || []), ""] }));
  const removeInstallCommand = (idx) => setEnv((f) => {
    const cmds = [...(f.installCommands || [])];
    cmds.splice(idx, 1);
    return { ...f, installCommands: cmds };
  });
  ...
  if (loading) {
    return html`<${Box} sx=${{ textAlign: "center", py: 4 }}><${CircularProgress} size=${28} /><//>`;
  }

  if (repos.length === 0) {
    return html`
      <${Alert} severity="info" sx=${{ mt: 2 }}>
        No repositories found in config. Add repos via the setup wizard or by editing <code>bosun.config.json</code>.
      <//>
    `;
  }

  const selectedTpl = templates.find((t) => t.id === env?.template);

  return html`
    <${Stack} spacing=${2.5}>
      <!-- Header -->
      <${Box}>
        <${Typography} variant="h6" gutterBottom>${iconText(":gear: Configure Environment")}<//>
        <${Typography} variant="body2" color="text.secondary">
          Set up install, build, test, start, and debug commands for each repository.
          Bosun uses these when bootstrapping worktrees and resolving <code>auto</code> commands in workflows.
        <//>
      <//>

      <!-- Repo picker + auto-detect -->
      <${Stack} direction="row" spacing=${1.5} alignItems="center" flexWrap="wrap">
        <${FormControl} size="small" sx=${{ minWidth: 240 }}>
          <${InputLabel}>Repository<//>
          <${Select} value=${selectedRepo} onChange=${handleRepoChange} label="Repository">
            ${repos.map((r) => {
              const name = r.name || r.slug || "";
              return html`<${MenuItem} key=${name} value=${name}>${r.slug || name}<//>`;
            })}
          <//>
        <//>
        <${Button}
          variant="outlined" size="small"
          startIcon=${detecting ? html`<${CircularProgress} size=${14} />` : null}
          onClick=${handleAutoDetect}
          disabled=${detecting || !selectedRepo}
        >
          ${detecting ? "Detecting…" : iconText("✨ Auto-detect")}
        <//>
        ${detectedInfo?.template && html`
          <${Chip}
            label=${`${detectedInfo.template.icon || ""} Detected: ${detectedInfo.template.label}`}
            color="success" size="small" variant="outlined"
          />
        `}
        ${detectedInfo?.confidence && html`
          <${Chip} label=${`Confidence: ${detectedInfo.confidence}`} size="small" variant="outlined"
            color=${detectedInfo.confidence === "high" ? "success" : detectedInfo.confidence === "medium" ? "warning" : "default"}
          />
        `}
      <//>
      ...
      <${FormControl} fullWidth size="small">
        ...
        <${Select}
          ...
          displayEmpty
        >
          <${MenuItem} value=""><em>— No template —</em><//>
          ${templateGroups.map((group) => [
            html`<${MenuItem} key=${`hdr-${group}`} value="" disabled sx=${{ fontWeight: 700, opacity: 0.7, fontSize: "0.78em", textTransform: "uppercase", color: "text.secondary" }}>${group}<//>`,
            ...templates.filter((t) => t.group === group).map((t) => html`
              <${MenuItem} key=${t.id} value=${t.id}>${t.icon || ""} ${t.label}<//>
            `),
          ])}
        <//>
      <//>
      ${selectedTpl && html`
        <${Typography} variant="caption" color="text.secondary" sx=${{ mt: -1.5 }}>${selectedTpl.description}<//>
      `}
      ...
      <!-- Install Commands -->
      <${Box}>
        <${Typography} variant="subtitle2" sx=${{ mb: 1 }}>Install Commands<//>
        <${Typography} variant="caption" color="text.secondary" sx=${{ display: "block", mb: 1 }}>
          Commands to install dependencies in each new worktree (run in order).
        <//>
        <${Stack} spacing=${1}>
          ${(env?.installCommands || []).map((cmd, i) => html`
            <${Stack} key=${i} direction="row" spacing=${1} alignItems="center">
              <${TextField} size="small" fullWidth
                value=${cmd}
                onInput=${updateInstallCommand(i)}
                placeholder="e.g. npm install"
                InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
              />
              <${IconButton} size="small" onClick=${() => removeInstallCommand(i)} color="error">
                <span>✕</span>
              <//>
            <//>
          `)}
          <${Button} variant="outlined" size="small" onClick=${addInstallCommand} sx=${{ alignSelf: "flex-start" }}>
            ${iconText("➕ Add Command")}
          <//>
        <//>
      <//>
      ...
      <${TextField} size="small" fullWidth
        ...
        value=${env?.worktreeSetupScript || ""}
        onInput=${updateEnvField("worktreeSetupScript")}
        placeholder="e.g. node tools/install-git-hooks.mjs"
        helperText="Runs once in every new worktree before install commands."
        InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
      />
      ...
      <!-- Command fields -->
      <${Typography} variant="subtitle2">Commands<//>
      <${Stack} spacing=${1.5}>
        <${TextField} size="small" fullWidth label="Start Command"
          value=${env?.startCommand || ""}
          onInput=${updateEnvField("startCommand")}
          placeholder="e.g. npm run dev  or  docker compose up -d"
          helperText="Command to start the dev server or services for agent work."
          InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
        />
        <${TextField} size="small" fullWidth label="Build Command"
          value=${env?.buildCommand || ""}
          onInput=${updateEnvField("buildCommand")}
          placeholder='e.g. npm run build  or  "auto" to detect automatically'
          InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
        />
        <${TextField} size="small" fullWidth label="Test Command"
          value=${env?.testCommand || ""}
          onInput=${updateEnvField("testCommand")}
          placeholder='e.g. npm test  or  "auto" to detect automatically'
          InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
        />
        <${TextField} size="small" fullWidth label="Lint Command"
          value=${env?.lintCommand || ""}
          onInput=${updateEnvField("lintCommand")}
          placeholder='e.g. npm run lint  or  "auto" to detect automatically'
          InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
        />
        <${TextField} size="small" fullWidth label="Debug Command"
          value=${env?.debugCommand || ""}
          onInput=${updateEnvField("debugCommand")}
          placeholder="e.g. node --inspect dist/index.js"
          InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
        />
      <//>
      ...
      <!-- Shared Paths -->
      <${Box}>
        <${Typography} variant="subtitle2" sx=${{ mb: 0.5 }}>Shared Paths (symlinked into worktrees)<//>
        <${Typography} variant="caption" color="text.secondary" sx=${{ display: "block", mb: 1 }}>
          Paths symlinked from the repo root into each worktree to avoid redundant installs
          (e.g. <code>node_modules</code>, <code>vendor</code>, <code>.gradle</code>).
        <//>
        <${TextField} size="small" fullWidth
          value=${(env?.sharedPaths || []).join(", ")}
          onInput=${(e) => setEnv((f) => ({ ...f, sharedPaths: e.target.value.split(/[,\s]+/).map((p) => p.trim()).filter(Boolean) }))}
          placeholder="e.g. node_modules, .gradle, vendor/bundle"
          InputProps=${{ sx: { fontFamily: "monospace", fontSize: "0.85em" } }}
        />
      <//>
