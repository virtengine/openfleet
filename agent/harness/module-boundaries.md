...
- `agent/agent-pool.mjs`
  - Compatibility facade for pool/thread entrypoints and temporary harness launch bridge.
...
- `primary-agent.mjs` and `agent-pool.mjs` may call into `internal-harness-runtime.mjs`, `session-manager.mjs`, `thread-registry.mjs`, and `provider-kernel.mjs`.
