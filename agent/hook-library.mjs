...
/**
 * Enable all hooks in a category.
 * @param {string} rootDir
 * @param {string} category
 * @returns {{ success: boolean, enabled: number, total: number }}
 */
export function bulkEnableByCategory(rootDir, category) {
  const hooks = getHookCatalog({ category });
  if (hooks.length === 0) return { success: false, enabled: 0, total: 0 };
  const state = loadHookState(rootDir);
  let count = 0;
  for (const hook of hooks) {
    if (state.enabled[hook.id] !== true) {
      state.enabled[hook.id] = true;
      count++;
    }
  }
  if (count > 0) saveHookState(rootDir, state);
  return { success: true, enabled: count, total: hooks.length };
}

/**
 * Disable all non-core hooks in a category (core hooks need force=true).
 * @param {string} rootDir
 * @param {string} category
 * @param {boolean} [force=false]
 * @returns {{ success: boolean, disabled: number, skipped: string[], total: number }}
 */
export function bulkDisableByCategory(rootDir, category, force = false) {
  const hooks = getHookCatalog({ category });
  if (hooks.length === 0) return { success: false, disabled: 0, skipped: [], total: 0 };
  const state = loadHookState(rootDir);
  let count = 0;
  const skipped = [];
  for (const hook of hooks) {
    ...
    if (state.enabled[hook.id] !== false) {
      state.enabled[hook.id] = false;
      count++;
    }
  }
  if (count > 0) saveHookState(rootDir, state);
  return { success: true, disabled: count, skipped, total: hooks.length };
}

// ── Hook Overrides (user edits) ─────────────────────────────────────────────

const HOOK_OVERRIDES_FILE = "hook-overrides.json";

/**
 * Load all hook overrides for a workspace.
 * @param {string} rootDir
 * @returns {Record<string, object>}
 */
export function loadHookOverrides(rootDir) {
  const overridesFile = resolve(rootDir, ".bosun", HOOK_OVERRIDES_FILE);
  try {
    if (existsSync(overridesFile)) return JSON.parse(readFileSync(overridesFile, "utf8"));
  } catch { /* corrupt file */ }
  return {};
}

/**
 * Save an override for a specific hook (command, timeout, blocking, env).
 * @param {string} rootDir
 * @param {string} hookId
 * @param {object} overrides
 * @returns {{ success: boolean, warning?: string, overrides?: object }}
 */
export function saveHookOverride(rootDir, hookId, overrides) {
  const hook = getHookById(hookId);
  if (!hook) return { success: false, warning: `Unknown hook: ${hookId}` };
  const bosunDir = resolve(rootDir, ".bosun");
  mkdirSync(bosunDir, { recursive: true });
  const overridesFile = resolve(bosunDir, HOOK_OVERRIDES_FILE);
  let allOverrides = {};
  try {
    if (existsSync(overridesFile)) allOverrides = JSON.parse(readFileSync(overridesFile, "utf8"));
  } catch { /* corrupt file */ }
  const allowedKeys = ["command", "timeout", "blocking", "env"];
  const sanitized = {};
  for (const key of allowedKeys) {
    if (overrides[key] !== undefined) sanitized[key] = overrides[key];
  }
  if (Object.keys(sanitized).length === 0) {
    return { success: false, warning: "No valid override fields provided. Allowed: command, timeout, blocking, env" };
  }
  allOverrides[hookId] = { ...(allOverrides[hookId] || {}), ...sanitized, updatedAt: new Date().toISOString() };
  writeFileSync(overridesFile, JSON.stringify(allOverrides, null, 2) + "\n", "utf8");
  return { success: true, overrides: allOverrides[hookId] };
}

/**
 * Delete overrides for a specific hook (revert to defaults).
 * @param {string} rootDir
 * @param {string} hookId
 * @returns {{ success: boolean }}
 */
export function deleteHookOverride(rootDir, hookId) {
  const overridesFile = resolve(rootDir, ".bosun", HOOK_OVERRIDES_FILE);
  let allOverrides = {};
  try {
    if (existsSync(overridesFile)) allOverrides = JSON.parse(readFileSync(overridesFile, "utf8"));
  } catch { /* corrupt file */ }
  if (!(hookId in allOverrides)) return { success: true };
  delete allOverrides[hookId];
  writeFileSync(overridesFile, JSON.stringify(allOverrides, null, 2) + "\n", "utf8");
  return { success: true };
}

/**
 * Get a hook with user overrides applied.
 * @param {string} rootDir
 * @param {string} hookId
 * @returns {HookLibraryEntry|null}
 */
export function getHookWithOverrides(rootDir, hookId) {
  const hook = getHookById(hookId);
  if (!hook) return null;
  const overrides = loadHookOverrides(rootDir);
  const hookOverrides = overrides[hookId];
  if (!hookOverrides) return hook;
  return {
    ...hook,
    command: hookOverrides.command ?? hook.command,
    timeout: hookOverrides.timeout ?? hook.timeout,
    blocking: hookOverrides.blocking ?? hook.blocking,
    env: hookOverrides.env ? { ...hook.env, ...hookOverrides.env } : hook.env,
    _hasOverrides: true,
    _originalCommand: hook.command,
    _overrides: hookOverrides,
  };
}
