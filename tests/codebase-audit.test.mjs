...
describe("codebase audit CLI helpers", () => {
  ...
  it("fails conformity when risk warnings are missing", () => {
    writeRepoFile(
      "src/risky.mjs",
      [
        "// CLAUDE:SUMMARY Handles risky lazy loader responsibilities.",
        "let cached = null;",
        "export async function loadRisky() {",
        "  if (!cached) {",
        "    const { createClient } = await import('./dep.mjs');",
        "    cached = createClient();",
        "  }",
    ...
    const { report } = runAuditConformity(repoDir, { files: ["src/risky.mjs", "src/dep.mjs"] });
    expect(report.ok).toBe(false);
    expect(report.missingWarnings).toContain("src/risky.mjs");
  });
});
