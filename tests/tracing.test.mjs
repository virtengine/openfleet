...
import {
  getTracingState,
  recordAgentEvent,`r`n  recordAgentIntervention,`r`n  resetTracingForTests,
  setupTracing,
  traceAgentSession,
  traceLLMCall,
  traceTaskExecution,
  ...
  it("tracks session lifecycle and intervention metrics", async () => {
    ...
    await traceTaskExecution({ taskId: "task-session" }, async () => {
      await traceAgentSession({ sessionId: "session-life", taskId: "task-session", sdk: "codex" }, async () => {
        recordAgentIntervention("force_new_thread", { taskId: "task-session", sessionId: "session-life" });
      });
    });
    const tracingState = getTracingState();
    expect(tracingState.metrics.interventionsByType.force_new_thread).toBe(1);
    const sessionSpan = tracingState.testSpans.find((entry) => entry.name === "bosun.agent.session");
