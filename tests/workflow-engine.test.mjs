...
describe("Session chaining - action.run_agent", () => {
  ...
  it("enforces stable systemPrompt in strict cache anchor mode", async () => {
    vi.stubEnv("BOSUN_CACHE_ANCHOR_MODE", "strict");
    vi.resetModules();
    try {
      const { getNodeType: getNodeTypeReloaded } = await import("../workflow/workflow-nodes.mjs");
      const handler = getNodeTypeReloaded("action.run_agent");
      expect(handler).toBeDefined();

      const ctx = new WorkflowContext({
        taskId: "TASK-901",
        taskTitle: "Anchor prompt cache",
        branch: "feat/cache-anchor",
        worktreePath: "/tmp/cache-wt",
        repoRoot: "/tmp/repo",
      });
      ...
      const node = {
        id: "strict-cache-anchor",
        type: "action.run_agent",
        config: {
          prompt: "Implement requested task.",
          autoRecover: false,
          systemPrompt:
            "Task ID: TASK-901\nWorking Directory: /tmp/cache-wt\nCo-authored-by: bosun[bot] <bosun@virtengine.com>",
        },
      };
      ...
      expect(launchEphemeralThread.mock.calls[0][3]).toEqual(
        expect.objectContaining({
          systemPrompt: expect.stringContaining("# BOSUN AGENT SYSTEM DIRECTIVE"),
        }),
      );
      expect(launchEphemeralThread.mock.calls[0][3].systemPrompt).not.toContain("TASK-901");
      expect(launchEphemeralThread.mock.calls[0][3].systemPrompt).not.toContain("/tmp/cache-wt");
      expect(launchEphemeralThread.mock.calls[0][3].systemPrompt).not.toContain("Co-authored-by");
    } finally {
      vi.unstubAllEnvs();
    }
  });
