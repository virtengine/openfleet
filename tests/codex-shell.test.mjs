...
describe("codex-shell stream safeguards", () => {
  ...
  it("avoids reserved built-in openai provider ids for custom base URLs", async () => {
    ...
    const providerId = ctorOptions.config?.model_provider
      || Object.keys(ctorOptions.config?.model_providers || {})[0];
    expect(providerId).toBeDefined();
    expect(providerId).not.toBe("openai");
    expect(providerId).not.toBe("azure");
    expect(ctorOptions.config?.model_providers?.[providerId]).toBeDefined();
  });
